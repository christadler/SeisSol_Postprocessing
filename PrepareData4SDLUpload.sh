...
#sdl_dir=/hppfs/scratch/0A/di35ban/AltoTiberina/sdl_10bigRuns
sdl_dir=/hppfs/scratch/0A/di35ban/AltoTiberina/sdl_10bigRuns_wSmallerFaultOutp
mkdir -pv $sdl_dir
...
for i in {0000..0009}; do
        ...
        # cp $i-fault.xdmf $sdl_dir/$i
        # cp -r $i-fault_cell $sdl_dir/$i
        # cp -r $i-fault_vertex $sdl_dir/$i
        cp $i\_5s-fault.xdmf $sdl_dir/$i
        cp $i\_5s-fault.h5 $sdl_dir/$i
